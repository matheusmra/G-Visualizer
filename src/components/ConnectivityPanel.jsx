      ...
      {result && (
        <div className="flex flex-col gap-4">
          ...
          {!isDirected && (
            <div>
              ...
              {result.bridges.length === 0 ? (
                <p className="text-xs text-gray-400 dark:text-gray-500 italic">Nenhuma ponte detectada.</p>
              ) : (
                <div className="flex flex-col gap-1">
                  {result.bridges.map(edgeId => {
                    ...
                    const label = edge
                      ...
                      : edgeId;
                    return (
                      <div key={edgeId} className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-red-950/40 border border-red-800/40">
                        <span className="w-2 h-2 rounded-full bg-red-400 shrink-0" />
                        <span className="text-xs text-red-300 font-mono">{label}</span>
                      </div>
                    );
                  })}
                </div>
          ...
          {!isDirected && (
            <div>
              ...
              {result.articulationPoints.length === 0 ? (
                <p className="text-xs text-gray-400 dark:text-gray-500 italic">Nenhum ponto de articulação detectado.</p>
              ) : (
                <div className="flex flex-wrap gap-1.5">
                  {result.articulationPoints.map(id => (
                    <span
                      ...
                      style={{ backgroundColor: '#431407cc', borderColor: '#f97316', color: '#fdba74' }}
                    >
                      {id}
                    </span>
                  ))}
          ...
          <div className="rounded-xl bg-gray-100 dark:bg-gray-800/60 border border-gray-200 dark:border-gray-700 p-3 flex flex-col gap-1.5">
            <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 dark:text-gray-500 mb-1">Legenda</p>
            {result.groups.slice(0, 3).map((_, i) => {
              ...
              return (
                <div key={i} className="flex items-center gap-2 text-xs">
                  ...
                  <span className="text-gray-500 dark:text-gray-400">{groupLabel} {i + 1}</span>
                </div>
              );
            })}
            ...
            {!isDirected && (
              <>
                <div className="flex items-center gap-2 text-xs mt-1">
                  ...
                  <span className="text-gray-500 dark:text-gray-400">Ponte</span>
                </div>
                <div className="flex items-center gap-2 text-xs">
                  <span className="w-3 h-3 rounded-full border-2 shrink-0" style={{ borderColor: '#f97316', backgroundColor: '#1e293b' }} />
                  <span className="text-gray-500 dark:text-gray-400">Ponto de Articulação</span>
                </div>
              </>
            )}
          </div>
